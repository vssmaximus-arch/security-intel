...
# Aggressive Blocklist (Kill cleanup/residents returning stories)
BLOCKLIST = [
...
def ai_process(model, title, summary):
    ...
    Input: "{title} - {summary}"
    Rules:
    1. REJECT (keep=false): Sports, Entertainment, Politics (unless violent), Post-disaster cleanup/recovery stories (e.g. residents returning, cleanup).
    ...
    for url in FEEDS:
        try:
            ...
            for e in f.entries[:15]:
                ...
                if is_blocked(full_text): continue
