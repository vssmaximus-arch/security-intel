...
def ask_gemini_analyst(title, snippet):
    ...
    Task: Filter and Categorize.
    Headline: "{title}"
    Snippet: "{snippet}"
    INSTRUCTIONS:
    1. Discard Politics/Business/Sports/Celebrity/General Crime.
    2. Keep Security/Logistics/Disaster/Cyber.
    OUTPUT JSON: {{ "category": "Physical Security"|"Cyber"|"Logistics"|"Weather/Event"|"Irrelevant", "severity": 1-3, "clean_title": "Professional Title", "summary": "1 sentence impact", "region": "Global", "lat": 0.0, "lon": 0.0 }}
    """
    try:
        time.sleep(1.5)
...
def fetch_news():
    ...
    with open(DB_PATH, "w") as f: json.dump(clean_list, f, indent=2)
