...
def generate_html(profile, items):
    ...
    html = f"<html><head><title>{profile['title']}</title><style>body{{font-family:sans-serif;padding:20px}}</style></head><body><h1>{profile['title']}</h1><p>Generated: {date_str}</p><hr>"
    if not items: html += "<p>No critical events.</p>"
    for item in items:
        html += f"<h3>{item['title']}</h3><p>{item['snippet']}</p><small>{item['source']} | {item.get('date_str', 'N/A')}</small><hr>"
    html += "</body></html>"
    return html
