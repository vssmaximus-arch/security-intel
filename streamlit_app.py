...
/* Use the header-marker div to style this vertical block as the header strip */
...
# ---------------- HEADER ----------------
with st.container():
    # marker for CSS :has() to grab this block as header
    st.markdown('<div class="header-marker"></div>', unsafe_allow_html=True)

    # Grid: Logo(2.5) | Spacer(0.5) | Pills(4.5) | Clock(2) | Button(1.5)
...
with main_col:
    ...
    st.info(f"MAP PLACEHOLDER – {selected_region.upper()}")
    ...
    # STREAM CONTENT
    st.info(f"Stream placeholder – filter: {selected_region}.")
...
with side_col:
    ...
    with st.container():
        ...
        st.caption("Travel card placeholder.")
    ...
    with st.container():
        ...
        st.selectbox(
            "Radius", ["5 KM (Default)", "10 KM", "25 KM", "50 KM"],
            label_visibility="collapsed",
        )
        st.caption("Currently no alerts in proximity to Dell sites.")
    ...
    with st.container():
        ...
        st.caption("Risk filter placeholder.")
