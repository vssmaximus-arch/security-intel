...
## 🎯 Current Milestone: S2.5 Fixed & Expanded — Maritime Tracking + AISStream Prep
> **Active Status**: Phase 2 complete; Phase 3 next (S3.1 DEP_APPROVAL required)
> **Current Context**: S2.5 hotfix — root cause of HTTP 400 was `app.js` sending `body.icao24` while worker read `body.id`; fixed with unified `id || icao24 || mmsi` read; GET now returns `{ watchlist: [{id, type, label}] }` instead of raw array; test-alert now handled explicitly; all 400/500 responses return JSON `{ ok, error }`; Flight/Vessel type toggle added to drawer input row; vessel cards render "Track via AIS" deep-link to `vesselfinder.com` instead of Live Radar; `fetchAisStreamData(mmsi)` stub added (ready for AISStream.io key); ICAO24 maxlength raised to 9 for MMSI support. Commits: `9acaa61` (root) / `5dcca08` (worktree).
> **Next Session Tip**: Deploy worker (`wrangler publish`), open **Logistics** drawer → **Watchlist** tab, toggle to **Flight**, type `a12bc3`, click **Add** — confirm no 400 and card appears with Live Radar button. Toggle to **Vessel**, type a vessel name, click **Add** — confirm card appears with "Track via AIS" deep-link. Click **Test Alert** — confirm inbox receives test email. Next: secure `AISSTREAM_API_KEY` in wrangler secrets for live vessel dots.
